...
class MarketIntelligenceCollector:
    ...
    def search_web_and_extract(
        ...
        if not self.serper_api_key:
            ...
            return pd.DataFrame()
        return asyncio.run(
            self._async_search_and_extract(query, search_type, num_results, date_filter)
        )

    async def _async_search_and_extract(
        self, query: str, search_type: str, num_results: int, date_filter: str
    ) -> pd.DataFrame:
        endpoint = (
        ...
        # --- Native date filtering flags for Serper ---
        date_filter_map = {"d": "qdr:d", "w": "qdr:w", "m": "qdr:m"}
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                resp = await client.post(
                    endpoint, headers=self.serper_headers, json=payload
                )
                resp.raise_for_status()
            results = resp.json().get(
                "news" if search_type == "news" else "organic", []
            )
            if not results:
